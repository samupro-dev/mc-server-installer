...
do
  ...
  case "$choiceupdater" in
    n|N|no|No|NO) break ;;
    y|Y|yes|Yes|YES) bash <(wget -O - 'https://raw.githubusercontent.com/samupro-dev/') ;;
...
function flamecord {
  ...
  read flamecordmem
  echo -e -n "${CYAN} Enter the location of the server folder. (ex. /root/bungee): ${COLOR_NULL}"
  read flamecordfolder
  mkdir ${flamecordfolder}
  cd ${flamecordfolder}
  echo -e "${CYAN} Downloading the jar file. . . ${COLOR_NULL}"
  wget https://2lstudios.dev/mirror/FlameCord/FlameCord-0.4.6.jar
  mv FlameCord-0.4.6.jar FlameCord.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${flamecordmem}M -jar FlameCord.jar" > RunMe.sh
  chmod +x RunMe.sh
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  echo -e "${LIGHT_GREEN} Your server was successfully installed!\n   ${CYAN}* Version: ${WHITE}FlameCord\n   ${CYAN}* Location: ${WHITE}${flamecordfolder}\n   ${CYAN}* RAM: ${WHITE}${flamecordmem}M ${COLOR_NULL}"
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  exit
}
...
function hexacord {
  ...
  echo -e -n "${CYAN} Enter the location of the server folder. (ex. /root/bungee): ${COLOR_NULL}"
  read hexacordfolder
  mkdir ${hexacordfolder}
  cd ${hexacordfolder}
  echo -e "${CYAN} Downloading the jar file. . . ${COLOR_NULL}"
  wget https://github.com/HexagonMC/BungeeCord/releases/releases/latest/download/BungeeCord.jar
  mv BungeeCord.jar hexacord.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${hexacordmem}M -jar hexacord.jar" > RunMe.sh
  chmod +x RunMe.sh
  echo -e " "
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  echo -e "${LIGHT_GREEN} Your server was successfully installed!\n   ${CYAN}* Version: ${WHITE}HexaCord\n   ${CYAN}* Location: ${WHITE}${hexacordfolder}\n   ${CYAN}* RAM: ${WHITE}${hexacordmem}M ${COLOR_NULL}"
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  exit
}
...
function spigot {
  ...
  SPIGOTTYPE=("Spigot" "Paper" "Yatopia" "Tuinity" "TacoSpigot" "Glowstone" "Cancel")
  echo -e "${CYAN} Select the type of fork that suits you best! ${COLOR_NULL}"
...
function tuinity {
  ...
  echo -e -n "${CYAN} Enter the RAM to be assigned in MB (ex. 512): ${COLOR_NULL}"
  read tuinitymem
  echo -e -n "${CYAN} Enter the location of the server folder. (ex. /root/tuinity): ${COLOR_NULL}"
  read tuinityfolder
  mkdir ${tuinityfolder}
  cd ${tuinityfolder}
  echo -e -n "${YELLOW} Enter the port of the server. (ex. 25599): ${COLOR_NULL}"
  read tuinityport
  echo -e "${YELLOW} I am setting up the server port. . . ${COLOR_NULL}"
  echo "server-port=${tuinityport}" > server.properties
  echo -e "${YELLOW} The eula file has been created. ${COLOR_NULL}"
  echo "eula=true" > eula.txt
  tuinityversion
}
...
function tuinity1164 {
  echo -e " "
  cd ${tuinityfolder}
  wget https://ci.codemc.io/job/Spottedleaf/job/Tuinity/lastSuccessfulBuild/artifact/tuinity-paperclip.jar
  mv tuinity-paperclip.jar tuinity-1.16.4.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${tuinitymem}M -jar tuinity-1.16.4.jar nogui" > RunMe.sh
  chmod +x RunMe.sh
  tuinitysuccess
}
...
function tuinitysuccess {
  echo -e " "
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  echo -e "${LIGHT_GREEN} Your server was successfully installed!\n   ${CYAN}* Version: ${WHITE}Tuinity ${TUINITYVERSIONSEL}\n   ${CYAN}* Location: ${WHITE}${tuinityfolder}\n   ${CYAN}* RAM: ${WHITE}${tuinitymem}M\n   ${CYAN}* Port: ${WHITE}${tuinityport} ${COLOR_NULL}"
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  exit
}
...
function tacospigot {
  ...
  echo -e -n "${CYAN} Enter the RAM to be assigned in MB (ex. 512): ${COLOR_NULL}"
  read tacospigotmem
  echo -e -n "${CYAN} Enter the location of the server folder. (ex. /root/tacospigot): ${COLOR_NULL}"
  read tacospigotfolder
  mkdir ${tacospigotfolder}
  cd ${tacospigotfolder}
  echo -e -n "${YELLOW} Enter the port of the server. (ex. 25599): ${COLOR_NULL}"
  read tacospigotport
  echo -e "${YELLOW} I am setting up the server port. . . ${COLOR_NULL}"
  echo "server-port=${tacospigotport}" > server.properties
  echo -e "${YELLOW} The eula file has been created. ${COLOR_NULL}"
  echo "eula=true" > eula.txt
  tacospigotversion
}

function tacospigotversion {
  TACOSPIGOTVERSION=("1.12.2" "1.11.2" "1.8.8" "Cancel")
  echo -e "${CYAN} Select the server version. ${COLOR_NULL}"
  select TACOSPIGOTVERSIONSEL in "${TACOSPIGOTVERSION[@]}"; do
    case "$REPLY" in
    1) tacospigot1122 ;;
    2) tacospigot1112 ;;
    3) tacospigot188 ;;
...
function tacospigot1122 {
  echo -e " "
  cd ${tacospigotfolder}
  wget https://yivesmirror.com/files/tacospigot/TacoSpigot-1.12.2-b114.jar
  mv TacoSpigot-1.12.2-b114.jar tacospigot-1.12.2.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${tacospigotmem}M -jar tacospigot-1.12.2.jar nogui" > RunMe.sh
  chmod +x RunMe.sh
  tacospigotsuccess
}

function tacospigot1112 {
  echo -e " "
  cd ${tacospigotfolder}
  wget https://yivesmirror.com/files/tacospigot/TacoSpigot-1.11.2-b102.jar
  mv TacoSpigot-1.11.2-b102.jar tacospigot-1.11.2.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${tacospigotmem}M -jar tacospigot-1.11.2.jar nogui" > RunMe.sh
  chmod +x RunMe.sh
  tacospigotsuccess
}

function tacospigot188 {
  echo -e " "
  cd ${tacospigotfolder}
  wget https://yivesmirror.com/files/tacospigot/TacoSpigot-1.8.8.jar
  mv TacoSpigot-1.8.8.jar tacospigot-1.8.8.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${tacospigotmem}M -jar tacospigot-1.8.8.jar nogui" > RunMe.sh
  chmod +x RunMe.sh
  tacospigotsuccess
}

function tacospigotsuccess {
  echo -e " "
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  echo -e "${LIGHT_GREEN} Your server was successfully installed!\n   ${CYAN}* Version: ${WHITE}TacoSpigot ${TACOSPIGOTVERSIONSEL}\n   ${CYAN}* Location: ${WHITE}${tacospigotfolder}\n   ${CYAN}* RAM: ${WHITE}${tacospigotmem}M\n   ${CYAN}* Port: ${WHITE}${tacospigotport} ${COLOR_NULL}"
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  exit
}
...
function glowstone1122 {
  echo -e " "
  cd ${glowstonefolder}
  wget https://github.com/GlowstoneMC/Glowstone/releases/latest/download/glowstone.jar
  mv glowstone.jar glowstone-1.12.2.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${glowstonemem}M -jar glowstone-1.12.2.jar nogui" > RunMe.sh
  chmod +x RunMe.sh
  glowstonesuccess
}
...
function modded {
  ...
  MODDEDTYPE=("Forge" "Sponge" "Magma" "Cancel")
  echo -e "${CYAN} Select the type of fork that suits you best! ${COLOR_NULL}"
...
function magma {
  ...
  echo -e -n "${CYAN} Enter the RAM to be assigned in MB (ex. 512): ${COLOR_NULL}"
  read magmamem
  echo -e -n "${CYAN} Enter the location of the server folder. (ex. /root/magma): ${COLOR_NULL}"
  read magmafolder
  mkdir ${magmafolder}
  cd ${magmafolder}
  echo -e -n "${YELLOW} Enter the port of the server. (ex. 25599): ${COLOR_NULL}"
  read magmaport
  echo -e "${YELLOW} I am setting up the server port. . . ${COLOR_NULL}"
  echo "server-port=${magmaport}" > server.properties
  magmaversion
}

function magmaversion {
  MAGMAVERSION=("1.12" "Cancel")
  echo -e "${CYAN} Select the server version. ${COLOR_NULL}"
...
function magma112 {
  ...
  cd ${nukkitxfolder}
  wget https://ci.opencollab.dev/job/NukkitX/job/Nukkit/job/master/lastSuccessfulBuild/artifact/target/nukkit-1.0-SNAPSHOT.jar
  mv nukkit-1.0-SNAPSHOT.jar magma-1.12.jar
  echo -e "${YELLOW} The startup file has been created. ${COLOR_NULL}"
  echo "java -Xms128M -Xmx${nukkitxmem}M -jar magma-1.12.jar nogui" > RunMe.sh
  chmod +x RunMe.sh
  nukkitxsuccess
}
...
function magmasuccess {
  echo -e " "
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  echo -e "${LIGHT_GREEN} Your server was successfully installed!\n   ${CYAN}* Version: ${WHITE}Magma\n   ${CYAN}* Location: ${WHITE}${magmafolder}\n   ${CYAN}* RAM: ${WHITE}${magmamem}M\n   ${CYAN}* Port: ${WHITE}${magmaport} ${COLOR_NULL}"
  echo -e "${LIGHT_PURPLE}_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_/-/_"
  exit
}
